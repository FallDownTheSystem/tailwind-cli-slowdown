...
async function build() {
    ...
    async function startWatcher() {
        ...
        watcher.on('change', async (file)=>{
            console.error(`Watcher detected a change in ${file}`);
            if (contextDependencies.has(file)) {
                ...
                console.error('Adding to the chain', ++chainLength);
                chain = chain.then(async () =>{
                    changedContent.push({
                        content: _fs.default.readFileSync(_path.default.resolve(file), 'utf8'),
                        extension: _path.default.extname(file).slice(1)
                    ...
                    await rebuild(config1);
                    console.error('Removing from the chain', --chainLength);
                });
            }
        });
        watcher.on('add', async (file)=>{
            console.error(`Watcher adding a file: ${file}`);
            chain = chain.then(async ()=>{
                changedContent.push({
                    content: _fs.default.readFileSync(_path.default.resolve(file), 'utf8'),
                    extension: _path.default.extname(file).slice(1)
